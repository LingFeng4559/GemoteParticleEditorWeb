...
export function buildEditorAnnotationLines(state) {
    return [
        `${EDITOR_TAG} ${JSON.stringify({ version: 3, timelineDuration: state.timelineDuration || 80, timelineFrameInterval: state.timelineFrameInterval || 1, timelineFrameCount: state.timelineFrameCount || 81, timelineRetimingVersion: 1, checksum: calculateEditorChecksum(state.drawingGroups) })}`,
...
export function parseAnnotatedYml(text) {
    ...
    return {
        ...
        settings: { loop, head, timelineDuration: editor?.timelineDuration || 80, timelineFrameInterval: editor?.timelineFrameInterval || 1, timelineFrameCount: editor?.timelineFrameCount || Math.round((editor?.timelineDuration || 80) / (editor?.timelineFrameInterval || 1)) + 1, timelineRetimingVersion: editor?.timelineRetimingVersion || 0 },
